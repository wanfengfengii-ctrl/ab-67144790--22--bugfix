...
- 路径搜索在**整图**上进行（按“证书数最少、再按叶→根指纹序列字典序”的
  全局顺序枚举简单路径，**不设深度上限**：长链与短链一视同仁），
  但**不会先固定最短链再查撤销**：
  每条到达锚的完整路径都要同时通过密码学、层级、pathLen、名称约束、
  策略、EKU 和**双时态撤销**裁决。
- 环用“路径上集合”阻断：只枚举简单路径，可达候选空间有限，
  搜索在穷尽该空间后必然终止（有环图与交叉签发图均如此）。
- 规模友好：按主体名建桶（封存时用零密码学解析从 DER 抽取 Name 建索引），
  裁决时**惰性解析**证书——100k 张无关证书的图中裁决少量叶证书，
  不会反复全量解析 DER。
- 多路径唯一决胜：**证书数最少；并列取叶→根 DER SHA-256 指纹序列字典序最小**。
- 无有效路径时返回**覆盖所有可达候选分支**的拒绝证明：只有在真正穷尽
  可达候选状态之后才返回 `NO_PATH_TO_ANCHOR`。证明
